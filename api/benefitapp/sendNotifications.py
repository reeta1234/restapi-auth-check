...
class IOSPushNotifications:
    ...
    def send_push(self,**kwargs):
        try:
            ...
            s = socket.socket()
            sock = ssl.wrap_socket(s, ssl_version=ssl.PROTOCOL_SSLv3, certfile=self.__certificate)
            sock.connect(apnHost)
            ...
            payloadstr = json.dumps(payload, separators=(',',':'))
            payloadLen = len(payloadstr)
            fmt = "!cH32sH{0:d}s" % payloadLen
            command = '\x00'
            msg = struct.pack(fmt, command, len(token), token, payloadLen, payloadstr)
            sock.write(msg)
            sock.close()            
            return {'status':'success','response':'ios_push_sent'}
        except Exception as e:
