        ...
        try:
            ...
            if settings.IS_EXIST(settings.BASE_DIR+"/docs"):
                for i in os.listdir(settings.BASE_DIR+"/docs"):
                    if i.endswith('.pdf'):
                        data = open(settings.BASE_DIR+"/docs/sample.pdf", "r",errors='ignore').read()
            ...
            return json_response({"data":'encoded','msg':'data' , "status66":status.HTTP_300_MULTIPLE_CHOICES})
        except TypeError as e:
            return json_response({"data":e , "status66":status.HTTP_300_MULTIPLE_CHOICES})
    ...
    def get(self, request, format=None):
        ...
        with open(settings.BASE_DIR+"/docs/sample.pdf", 'rb') as pdf_file:
            ...
            for page_num in range(pdf_reader.numPages):
                ...
                print('********************************')
            print(tt)
        encoded = base64.urlsafe_b64encode(tt.encode('UTF-8')).decode('ascii')
        #tt = base64.decodestring(encoded.encode('UTF-8'))
        return json_response({"data":encoded,'msg':'data',"status":"success"})
